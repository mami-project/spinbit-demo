...
bool ts_all_estimate(vlib_main_t * vm, timestamp_observer_all_RTT_t * observer,
  ...
  if (src_port == init_src_port) {
    ...
    if (!ack_t) {
      tsecr--;
      ack_t = hash_get(observer->hash_ack_client, tsecr);
      if (!ack_t) {
        tsecr--;
        ack_t = hash_get(observer->hash_ack_client, tsecr);
      }
    ...
    if (!init_t_server) {
      tsecr--;
      init_t_server = hash_get(observer->hash_init_server, tsecr);
      if (!init_t_server) {
        tsecr--;
        init_t_server = hash_get(observer->hash_init_server, tsecr);
      }
    ...
    if (!ack_t) {
      tsecr--;
      ack_t = hash_get(observer->hash_ack_server, tsecr);
      if (!ack_t) {
        tsecr--;
        ack_t = hash_get(observer->hash_ack_server, tsecr);
      }
    ...
    if (!init_t_client) {
      tsecr--;
      init_t_client = hash_get(observer->hash_init_client, tsecr);
      if (!init_t_client) {
        tsecr--;
        init_t_client = hash_get(observer->hash_init_client, tsecr);
      }
...
void update_plus_rtt_estimate(vlib_main_t * vm, plus_observer_t * session,
        f64 now, u16 src_port, u16 init_src_port, u8 measurement, u32 psn,
  ...
  bool status = status_estimate(vm, &(session->status_spin_observer),
                now, src_port, init_src_port, spin, status_bits);
  bool all = ts_all_estimate(vm, &(session->ts_all_RTT_observer),
                now, src_port, init_src_port, psn, pse);
  ...
  if (pkt_count == 1){
    ...
    plus_printf(0, ",%s,%s", "status_data", "status_new");
    plus_printf(0, ",%s,%s", "all_ts_rtt", "all_ts_rtt_new");
    plus_printf(0, "\n");
  }

  /* If we have at least one update */
  if (new_rtt || status || all) {

    test_printf(1, "%u,%u,%u\n", pm->valid_edge, pm->invalid_edge, pm->valid_ts);
    /* Now print the actual data */
    if (src_port != init_src_port) {
      plus_printf(0, "%.*lf,%s,%u,%u,%u,%llu", TIME_PRECISION, now, "server", pkt_count, psn, pse, cat);
      
      plus_printf(0, ",%.*lf", 6, session->plus_single_observer.rtt_dst + session->plus_single_observer.rtt_src);
      plus_printf(0, ",%d", session->plus_single_observer.new_server);
      plus_printf(0, ",%.*lf", 6, session->status_spin_observer.rtt_server);
      plus_printf(0, ",%d", session->status_spin_observer.new_server);
      plus_printf(0, ",%.*lf", 6, session->ts_all_RTT_observer.rtt_server);
      plus_printf(0, ",%d", session->ts_all_RTT_observer.new_server);
      ...
      plus_printf(0, ",%.*lf", 6, session->plus_single_observer.rtt_src + session->plus_single_observer.rtt_dst);
      plus_printf(0, ",%d", session->plus_single_observer.new_client);
      plus_printf(0, ",%.*lf", 6, session->status_spin_observer.rtt_client);
      plus_printf(0, ",%d", session->status_spin_observer.new_client);
      plus_printf(0, ",%.*lf", 6, session->ts_all_RTT_observer.rtt_client);
      plus_printf(0, ",%d", session->ts_all_RTT_observer.new_client);
...
u32 create_session(sup_protocols_t p_type) {
  ...
  switch (p_type) {
    ...
    case P_PLUS:
      ...
      session->plus->status_spin_observer.spin_client = SPIN_NOT_KNOWN;
      session->plus->status_spin_observer.spin_server = SPIN_NOT_KNOWN;
      //session->plus->plus_single_observer.new_server = false;
      //session->plus->plus_single_observer.new_client = false;
      session->plus->ts_all_RTT_observer.hash_init_client =
        hash_create(0, sizeof(time_test_t*));
      session->plus->ts_all_RTT_observer.hash_init_server =
        hash_create(0, sizeof(time_test_t*));
      session->plus->ts_all_RTT_observer.hash_ack_client =
        hash_create(0, sizeof(time_test_t*));
      session->plus->ts_all_RTT_observer.hash_ack_server =
        hash_create(0, sizeof(time_test_t*));
    break;
...
void clean_session(u32 index)
{
  ...
  switch (session->p_type) {
    ...
    case P_PLUS:
      hash_free(session->plus->ts_all_RTT_observer.hash_init_client);
      hash_free(session->plus->ts_all_RTT_observer.hash_init_server);
      hash_free(session->plus->ts_all_RTT_observer.hash_ack_client);
      hash_free(session->plus->ts_all_RTT_observer.hash_ack_server);
...
void test_printf (int flush, char *fmt, ...) {
  ...
  if (output_file == NULL){
    output_file = fopen("/vagrant/spinbit_plus_counters.out", "w");
  }
  fprintf(output_file, "%s", s);
...
void plus_printf (int flush, char *fmt, ...) {
  ...
  if (output_file == NULL){
    output_file = fopen("/vagrant/spinbit_plus_printf.out", "w");
  }
  fprintf(output_file, "%s", s);
